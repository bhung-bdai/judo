...
SPOT_TASKS = [
    ...
    (SpotHandNavigate, 10),  # use_arm=True
    (SpotTireUpright, 17),  # use_arm=True, use_legs=True: 3+7+6+1
    (SpotTireDrop, 10),  # inherits from TireUpright but use_legs=False
    (SpotTireNavigate, 3),  # use_arm=False
    (SpotTireRoll, 11),  # use_arm=True, use_gripper=True
    (SpotTireDrag, 10),  # use_arm=True
    (SpotBoxPush, 10),  # use_arm=True
    (SpotBoxWithHandlePush, 10),  # use_arm=True
    (SpotSmallBoxPush, 10),  # use_arm=True
    (SpotBarbellLift, 11),  # use_arm=True, use_gripper=True
    (SpotTableDrag, 11),  # use_arm=True, use_gripper=True
    (SpotWheelRimRoll, 11),  # use_arm=True, use_gripper=True
    (SpotYellowChairPush, 10),  # use_arm=True
    (SpotYellowChairRampPush, 10),  # use_arm=True
    (SpotTireStack, 10),  # use_arm=True, use_torso=False
    (SpotTireRackInsert, 10),  # use_arm=True
]


@pytest.fixture(params=SPOT_TASKS, ids=lambda x: x[0].__name__)
def spot_task_and_nu(request):
...
class TestSpotTaskInstantiation:
    ...
    def test_instantiation(self, spot_task_and_nu):
        task, _ = spot_task_and_nu
        assert task.model is not None
        assert task.data is not None

    def test_nu(self, spot_task_and_nu):
...
class TestSpotTaskReward:
    ...
    def test_reward_shape(self, spot_task_and_nu):
        task, _ = spot_task_and_nu
        batch_size = 4
        T = 5
        nq = task.model.nq
    ...
    def test_reward_finite(self, spot_task_and_nu):
        task, _ = spot_task_and_nu
        batch_size = 2
        T = 3
        nq = task.model.nq
...
class TestSpotTaskResetPose:
    ...
    def test_reset_pose_shape(self, spot_task_and_nu):
        task, _ = spot_task_and_nu
    ...
    def test_reset_pose_finite(self, spot_task_and_nu):
        task, _ = spot_task_and_nu
        if not hasattr(task, "reset_pose"):
            pytest.skip(f"{task.__class__.__name__} has no reset_pose")
        pose = task.reset_pose
