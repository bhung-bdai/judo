...
class SkillPolicyConfig(OptimizerConfig):
    ...
    onnx_session = onnxruntime.InferenceSession(str(onnx_model_path), providers=["CPUExecutionProvider"])
...
class SkillPolicyOptimizer(Optimizer[SkillPolicyConfig]):
    ...
    def run_policy(self, state: np.ndarray) -> np.ndarray:
        ...
        from judo.tasks.spot.spot_constants import ARM_UNSTOWED_POS, isaac_to_mujoco
