...
from judo_cpp import G1Rollout, SpotRollout, sim_g1, sim_spot
...
class RolloutBackend:
    ...
    def __init__(
        self,
        num_threads: int,
        backend: Literal["mujoco", "mujoco_spot", "mujoco_g1"],
    ...
    def setup_mujoco_g1_backend(self, num_threads: int, cutoff_time: float) -> None:
        ...
        self.rollout_obj = G1Rollout(nthread=num_threads, cutoff_time=cutoff_time)
        self.rollout_func = lambda m, d, x0, u: self.rollout_obj.rollout(m, d, x0, u)
...
class SimBackendG1:
    ...
    def sim(self, sim_model: MjModel, sim_data: MjData, sim_controls: np.ndarray) -> None:
        ...
        controls = self.task_to_sim_ctrl(sim_controls)
        controls = controls.flatten()
        self.previous_policy_output = sim_g1(sim_model, sim_data, x0, controls, self.previous_policy_output)
