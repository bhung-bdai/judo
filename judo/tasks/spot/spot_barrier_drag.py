...
class SpotBarrierDrag(SpotBase[SpotBarrierDragConfig]):
    ...
    def reward(
        ...
        object_fallen_reward = -config.w_object_fallen * (object_pos[..., 2] <= config.object_fallen_threshold).any(
            axis=-1
        )
        ...
        barrier_to_goal = (
            np.array(config.goal_position)[None, None, :2] - object_pos[..., 1:, :2]
        )  # Skip initial state to match sensors
    ...
    def success(
        self, model: MjModel, data: MjData, config: SpotBarrierDragConfig, metadata: dict[str, Any] | None = None
    ) -> bool:
