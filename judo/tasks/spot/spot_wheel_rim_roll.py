...
"""SpotWheelRimRoll task - roll a wheel rim to a goal location.

Adapted from starfish/dexterity/tasks/spot_wheel_rim_roll.py.
"""
...
from judo.tasks.spot.spot_constants import LEGS_STANDING_POS, RIM_RADIUS, STANDING_HEIGHT, Z_AXIS
from judo.utils.fields import np_1d_field

XML_PATH = str(MODEL_PATH / "xml" / "spot_wheel_rim" / "robot.xml")


@dataclass
class SpotWheelRimRollConfig(SpotBaseConfig):
    """Configuration for the SpotWheelRimRoll task."""
...
class SpotWheelRimRoll(SpotBase[SpotWheelRimRollConfig]):
    """Task getting Spot to roll a wheel rim to a desired goal location."""

    name: str = "spot_wheel_rim_roll"
    config_t: type[SpotWheelRimRollConfig] = SpotWheelRimRollConfig  # type: ignore[assignment]
    config: SpotWheelRimRollConfig

    def __init__(
        self,
        config: SpotWheelRimRollConfig | None = None,
    ) -> None:
        """Initialize the SpotWheelRimRoll task."""
        ...
        self.object_pose_idx = self.get_joint_position_start_index("wheel_rim_joint")
        ...
        self.object_vel_idx = self.model.jnt_dofadr[self.model.joint("wheel_rim_joint").id]
    ...
    def reward(
        ...
        """Reward function for the wheel rim roll task."""
        ...
        goal_reward = -self.config.w_goal * np.linalg.norm(
            object_pos - self.config.goal_position, axis=-1
        ).mean(-1)
        ...
        return (
            spot_fallen_reward + tire_fallen_reward + goal_reward
            + torso_proximity_reward + gripper_proximity_reward + controls_reward
            + tire_linear_velocity_reward + tire_angular_velocity_reward
        )

    @property
    def reset_pose(self) -> np.ndarray:
        """Reset pose for the wheel rim roll task."""
        standing_pose = np.array([0, 0, STANDING_HEIGHT])
        robot_radius = 1.0
        reset_pose = (np.random.rand(7) - 0.5) * 3.0
        reset_pose[2] = RIM_RADIUS
