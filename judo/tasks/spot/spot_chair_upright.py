...
@dataclass
class SpotChairUprightConfig(SpotBaseConfig):
    """Config for the spot chair uprighting task."""
...
class SpotChairUpright(SpotBase):
    """Task getting Spot to upright a chair."""
    ...
    def reward(
        ...
        """Reward function for the Spot chair uprighting task."""
        ...
        # Compute orientation reward (chair y-axis should be vertical, so z-component should be 1 or -1)
        ...
        goal_reward = -config.w_goal * np.linalg.norm(
            object_pos - np.array([0.0, 0.0, DEFAULT_CHAIR_HEIGHT])[None, None], axis=-1
        ).mean(-1)
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        object_pose = np.array([*object_pos, DEFAULT_CHAIR_HEIGHT, np.cos(np.pi / 4), np.sin(np.pi / 4), 0, 0])
    ...
    def success(
        self, model: MjModel, data: MjData, config: SpotChairUprightConfig, metadata: dict[str, Any] | None = None
    ) -> bool:
        """Check if the chair is upright (y-axis vertical)."""
        # Get chair y-axis sensor data for orientation check
