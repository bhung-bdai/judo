...
"""SpotBoxWithHandlePush task - push a box with handle to a goal.

Adapted from starfish/dexterity/tasks/spot_box_with_handle_push.py.
"""
...
from judo.tasks.spot.spot_constants import BOX_WITH_HANDLE_HALF_LENGTH, LEGS_STANDING_POS, STANDING_HEIGHT, Z_AXIS
from judo.utils.fields import np_1d_field

XML_PATH = str(MODEL_PATH / "xml" / "spot_box_with_handle" / "robot.xml")
...
@dataclass
class SpotBoxWithHandlePushConfig(SpotBaseConfig):
    """Configuration for the SpotBoxWithHandlePush task."""
    ...
    goal_position: np.ndarray = np_1d_field(
        np.array([0.0, 0.0, BOX_WITH_HANDLE_HALF_LENGTH]),
        names=["x", "y", "z"],
        mins=[-5.0, -5.0, 0.0],
        maxs=[5.0, 5.0, 3.0],
...
class SpotBoxWithHandlePush(SpotBase[SpotBoxWithHandlePushConfig]):
    """Task getting Spot to push a box with a handle to a desired goal location."""

    name: str = "spot_box_with_handle_push"
    config_t: type[SpotBoxWithHandlePushConfig] = SpotBoxWithHandlePushConfig  # type: ignore[assignment]
    config: SpotBoxWithHandlePushConfig

    def __init__(
        self,
        config: SpotBoxWithHandlePushConfig | None = None,
    ) -> None:
        """Initialize the SpotBoxWithHandlePush task."""
        super().__init__(model_path=XML_PATH, use_arm=True, config=config)
        self.body_pose_idx = self.get_joint_position_start_index("base")
        self.object_pose_idx = self.get_joint_position_start_index("box_with_handle_joint")
    ...
    def reward(
        ...
        """Reward function for the box with handle push task."""
        ...
        return (
            spot_fallen_reward + goal_reward + box_orientation_reward
            + torso_proximity_reward + gripper_proximity_reward + controls_reward
        )

    @property
    def reset_pose(self) -> np.ndarray:
        """Reset pose for the box with handle push task."""
        ...
        reset_object_pose = np.array([*object_xy, BOX_WITH_HANDLE_HALF_LENGTH, 1, 0, 0, 0])
        return np.array(
            [*np.random.randn(2), STANDING_HEIGHT, 1, 0, 0, 0, *LEGS_STANDING_POS, *self.reset_arm_pos, *reset_object_pose]
        )
