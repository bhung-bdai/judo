...
@dataclass
class SpotRackUprightConfig(SpotBaseConfig):
    """Config for the spot rack uprighting task."""
...
class SpotRackUpright(SpotBase):
    """Task getting Spot to upright a rack."""
    ...
    def reward(
        ...
        sensors: np.ndarray,
        controls: np.ndarray,
        config: SpotRackUprightConfig,
        system_metadata: dict[str, Any] | None = None,
    ) -> np.ndarray:
        """Reward function for the Spot rack uprighting task."""
        ...
        # Compute orientation reward (rack y-axis should be vertical, so z-component should be 1 or -1)
        ...
        goal_reward = -config.w_goal * np.linalg.norm(
            object_pos - np.array([0.0, 0.0, DEFAULT_RACK_HEIGHT])[None, None], axis=-1
        ).mean(-1)
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        object_pose = np.array([*object_pos, DEFAULT_RACK_HEIGHT, np.cos(np.pi / 4), 0, np.sin(np.pi / 4), 0])
    ...
    def success(
        self, model: MjModel, data: MjData, config: SpotRackUprightConfig, metadata: dict[str, Any] | None = None
    ) -> bool:
        """Check if the rack is upright (y-axis vertical)."""
        # Get rack y-axis sensor data for orientation check
