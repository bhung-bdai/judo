...
"""SpotSmallBoxPush task - push a small box to a goal.

Adapted from starfish/dexterity/tasks/spot_small_box_push.py.
"""
...
from judo.tasks.spot.spot_constants import LEGS_STANDING_POS, SMALL_BOX_HALF_LENGTH, STANDING_HEIGHT
from judo.utils.fields import np_1d_field

XML_PATH = str(MODEL_PATH / "xml" / "spot_small_box" / "robot.xml")
...
@dataclass
class SpotSmallBoxPushConfig(SpotBaseConfig):
    """Configuration for the SpotSmallBoxPush task."""
...
class SpotSmallBoxPush(SpotBase[SpotSmallBoxPushConfig]):
    """Task getting Spot to push a small box to a desired goal location."""

    name: str = "spot_small_box_push"
    config_t: type[SpotSmallBoxPushConfig] = SpotSmallBoxPushConfig  # type: ignore[assignment]
    config: SpotSmallBoxPushConfig

    def __init__(
        self,
        config: SpotSmallBoxPushConfig | None = None,
    ) -> None:
        """Initialize the SpotSmallBoxPush task."""
        super().__init__(model_path=XML_PATH, use_arm=True, config=config)
        self.body_pose_idx = self.get_joint_position_start_index("base")
        self.object_pose_idx = self.get_joint_position_start_index("small_box_joint")
    ...
    def reward(
        ...
        """Reward function for the small box push task."""
        ...
        return spot_fallen_reward + goal_reward + torso_proximity_reward + gripper_proximity_reward + controls_reward

    @property
    def reset_pose(self) -> np.ndarray:
        """Reset pose for the small box push task."""
        ...
        reset_object_pose = np.array([*object_xy, SMALL_BOX_HALF_LENGTH, 1, 0, 0, 0])
        return np.array(
