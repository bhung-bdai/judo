...
class SpotBarbell(SpotBase[SpotBarbellConfig]):
    ...
    def reward(
        ...
        gripper_proximity_reward = -config.w_gripper_proximity * np.linalg.norm(gripper_to_object, axis=-1).mean(
            axis=-1
        )
        ...
        gripper_barbell_orientation_reward = -config.w_gripper_barbell_orientation * (1.0 - np.abs(dot_product)).mean(
            axis=-1
        )
        ...
        is_grasping = (gripper_distance < config.grasp_distance_threshold) & (
            barbell_height > config.grasp_height_threshold
        )
