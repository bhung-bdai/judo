...
class SpotTireStack(SpotBase[SpotTireStackConfig]):
    ...
    def reward(
        ...
        gripper_proximity_reward = -config.w_gripper_proximity * np.linalg.norm(p_WG - p_W_gripper_des, axis=-1).mean(
            axis=-1
        )
    ...
    def success(
        self, model: MjModel, data: MjData, config: SpotTireStackConfig, metadata: dict[str, Any] | None = None
    ) -> bool:
