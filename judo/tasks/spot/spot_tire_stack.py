...
Adapted from starfish/dexterity/tasks/spot_tire_stack.py.
"""
...
XML_PATH = str(MODEL_PATH / "xml" / "spot_two_tires" / "robot.xml")
...
class SpotTireStack(SpotBase[SpotTireStackConfig]):
    ...
    def __init__(
        ...
        self.top_tire_pose_idx = self.get_joint_position_start_index("tire_rubber_joint_2")
        ...
        self.top_tire_vel_idx = self.model.jnt_dofadr[self.model.joint("tire_rubber_joint_2").id]
    ...
    def reward(
        ...
        return (
            tire_stack_xy_reward + tire_stack_z_reward + tire_orientation_reward
            + bottom_tire_velocity_reward + gripper_proximity_reward + torso_proximity_reward
            + spot_fallen_reward + gripper_height_penalty + controls_reward
        )
