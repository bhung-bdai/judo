...
def isaac_to_mujoco(var_isaac: np.ndarray) -> np.ndarray:
    ...
    return var_isaac[isaac_to_mujoco_list]
...
def mujoco_to_isaac(var_mujoco: np.ndarray) -> np.ndarray:
    ...
    return var_mujoco[mujoco_to_isaac_list]


# ---- Locomotion policy default joint offsets (for input normalization & target offset) ----
LOCOMOTION_DEFAULT_JOINTS_OFFSET = (
    0.0, 0.12, -0.12, 0.12, -0.12, -0.9, 0.5, 0.5, 0.5, 0.5,
    1.8, -1.0, -1.0, -1.0, -1.0, 0.0, -0.9, 0.0, -1.54,
)

LOCOMOTION_DEFAULT_LEGS_OFFSET = (
    0.12, -0.12, 0.12, -0.12, 0.5, 0.5, 0.5, 0.5, -1.0, -1.0, -1.0, -1.0,
)
