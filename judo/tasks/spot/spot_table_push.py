...
@dataclass
class SpotTablePushConfig(SpotBaseConfig):
    """Config for the spot table pushing task."""

    goal_position: np.ndarray = np_1d_field(
        np.array([0.0, 0.0, DEFAULT_TABLE_HEIGHT], dtype=np.float64),
        names=["x", "y", "z"],
        mins=[-5.0, -5.0, 0.0],
        maxs=[5.0, 5.0, 1.0],
        steps=[0.1, 0.1, 0.05],
        ...
        xyz_vis_defaults=[0.0, 0.0, DEFAULT_TABLE_HEIGHT],
    )


class SpotTablePush(SpotBase):
    """Task getting Spot to push a table to a goal location."""
    ...
    def __init__(self, model_path: str = XML_PATH) -> None:
        ...
        self.object_y_axis_idx = get_sensor_indices(self.model, "sensor_table_y_axis")
    ...
    def reward(
        ...
        sensors: np.ndarray,
        controls: np.ndarray,
        config: SpotTablePushConfig,
        system_metadata: dict[str, Any] | None = None,
    ) -> np.ndarray:
        """Reward function for the Spot table pushing task."""
        ...
        # Compute unit vector pointing from table to torso
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        object_pose = np.array([*object_pos, DEFAULT_TABLE_HEIGHT, 1, 0, 0, 0])
    ...
    def success(
        self, model: MjModel, data: MjData, config: SpotTablePushConfig, metadata: dict[str, Any] | None = None
    ) -> bool:
