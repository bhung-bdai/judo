...
class SpotConeUprightConfig(SpotBaseConfig):
    """Config for the spot cone uprighting task."""
...
class SpotConeUpright(SpotBase):
    """Task getting Spot to upright a cone."""
    ...
    def reward(
        ...
        """Reward function for the Spot cone uprighting task."""
        ...
        # Compute orientation reward (cone z-axis should be vertical, so z-component should be 1 or -1)
        ...
        goal_reward = -config.w_goal * np.linalg.norm(
            object_pos - np.array([0.0, 0.0, DEFAULT_CONE_HEIGHT])[None, None], axis=-1
        ).mean(-1)
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        object_pose = np.array([*object_pos, DEFAULT_CONE_HEIGHT, np.cos(np.pi / 4), np.sin(np.pi / 4), 0, 0])
    ...
    def success(
        self, model: MjModel, data: MjData, config: SpotConeUprightConfig, metadata: dict[str, Any] | None = None
    ) -> bool:
        """Check if the cone is upright (z-axis vertical)."""
        # Get cone z-axis sensor data for orientation check
        object_z_axis = data.sensordata[self.object_z_axis_idx]

        # Check if z-axis is vertical (z-component should be close to 1 or -1)
        orientation_error = np.abs(np.abs(object_z_axis[2]) - 1.0)
