...
XML_PATH = str(MODEL_PATH / "xml/g1/g1_box.xml")
...
# Arm usage configuration for box manipulation
USE_LEFT_ARM = False
USE_RIGHT_ARM = False
...
class G1BoxConfig(G1BaseConfig):
    """Config for the G1 box manipulation task."""
...
class G1Box(G1Base):
    """Task getting G1 to move a box to a desired goal location.

    Uses right arm for manipulation by default (USE_RIGHT_ARM=True).
    """
    ...
    def reward(
        ...
        """Reward function for the G1 box moving task."""
        ...
        # Box orientation reward - penalize if box is not upright
        box_orientation_reward = -config.w_orientation * np.abs(np.dot(object_y_axis, Z_AXIS) - 1.0).sum(axis=-1)
        ...
        # Compute l2 distance from hands to object - encourage getting hands close to box
    ...
    def reset_pose(self) -> np.ndarray:
        ...
        # Sample object position in an annulus around the robot

        object_pos = np.array([-1.0, 0])  # + np.random.randn(2) * 0.1
        reset_object_pose = np.array([*object_pos, 0.5, 1, 0, 0, 0])
