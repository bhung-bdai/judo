...
if TYPE_CHECKING:
    from judo.utils.mujoco_warp import RolloutBackend as WarpRolloutBackend


def _make_model_data_pairs(model: MjModel, num_pairs: int) -> tuple[MjModel, MjData]:
...
class Controller:
    ...
    def __init__(
        ...
        task: Task,
        optimizer: Optimizer,
        rollout_backend: Literal["mujoco"] | WarpRolloutBackend = "mujoco",
    ) -> None:
        ...
        self.model_data_pairs = _make_model_data_pairs(self.model, self.optimizer_cfg.num_rollouts)

        if isinstance(rollout_backend, str):
            from judo.utils.mujoco import RolloutBackend  # noqa: PLC0415

            # Initialize rollout backend (uses C++ policy backend if task provides one)
        ...
        # Track locomotion policy state for C++ extensions backend
...
class BatchedControllers:
    ...
    def __init__(
        ...
        task: Task,
        optimizer: Optimizer,
        rollout_backend: WarpRolloutBackend,
    ) -> None:
