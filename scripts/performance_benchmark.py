...
from judo.utils.mujoco import SimBackendG1, SimBackendSpot
...
def load_controller(
    ...
    Args:
        ...
        task_config_overrides: Dictionary of task config parameters to override
    ...
    if task_config_overrides:
        for key, value in task_config_overrides.items():
            if not hasattr(task_config, key):
                raise ValueError(f"Task config '{task_name}' does not have parameter '{key}'")
    ...
    if verbose:
        ...
        if optimizer_name == "skill_policy":
            print("  onnx_session: <InferenceSession>")
    ...
    for i in range(num_episodes):
        ...
        for step in tqdm(range(num_steps), desc=f"Episode {i + 1}/{num_episodes}", leave=False):
            ...
            if step % loop_steps_per_ctrl_step == 0:  # control step (updates control from spline)
                ...
                # For custom backend tasks (Spot/G1), we don't set ctrl directly - it's handled in the sim step
            ...
            if step % loop_steps_per_sim_step == 0:  # simulation step (updates simulation state)
                task.pre_sim_step()
                if use_custom_backend and custom_sim_backend is not None and current_action is not None:
                    # Use custom simulation backend (Spot or G1)
                ...
                curr_episode_results["rewards"].append(0.0)
...
def summarize_benchmark_results(all_results: dict[str, dict[str, list[dict[str, Any]]]]) -> None:
    ...
    for task_name, task_results in all_results.items():
        ...
        for optimizer_name, episode_results_for_task_opt_pair in task_results.items():
            ...
            print(
                f"    STD of episode length: {np.std([r['length'] for r in episode_results_for_task_opt_pair]):.4f} seconds"
            )
    print("=" * 80)
