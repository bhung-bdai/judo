...
def run_single_episode(
    ...
    Args:
        ...
        custom_sim_backend: Custom simulation backend (SimBackendSpot or SimBackendG1 if applicable)
        use_custom_backend: Whether to use a custom backend (Spot or G1)
...
def run_mpc(config: RunMPCConfig) -> list[dict]:
    ...
    is_g1_task = "g1" in config.init_task
    ...
    for episode_idx in range(config.num_episodes):
        episode_data = run_single_episode(
            ...
            custom_sim_backend=custom_sim_backend,
            use_custom_backend=use_custom_backend,
    ...
    if config.save_results:
        ...
        h5_filename = os.path.join(output_dir, "trajectories.h5")
